...
export class EditSpecialtyComponent implements OnInit {
  ...
  confirmDelete(): void {
    if (this.sectionIdToDelete !== null) {
      this.apiService.delete(`Section/DeleteSection/${this.sectionIdToDelete}`).subscribe({
        next: () => {
          this.sections = this.sections.filter(section => section.id !== this.sectionIdToDelete);
